...
class SettingsConfig(QObject):
    ...
    def __init__(self):
        ...
        # General settings
        self.output_directory = ConfigItem(
            "General", "OutputDirectory", str(Path.home() / "Downloads"))
        self.auto_cleanup = ConfigItem(
            "General", "AutoCleanup", True, BoolValidator())
        self.auto_start_downloads = ConfigItem(
            "General", "AutoStartDownloads", True, BoolValidator())
        self.check_updates = ConfigItem(
            "General", "CheckUpdates", True, BoolValidator())
        self.enable_notifications = ConfigItem(
            "General", "EnableNotifications", True, BoolValidator())
        self.max_recent_files = RangeConfigItem(
            "General", "MaxRecentFiles", 10, RangeValidator(0, 50))
        self.max_concurrent_downloads = RangeConfigItem(
            "General", "MaxConcurrentDownloads", 3, RangeValidator(1, 10))
        self.retry_attempts = RangeConfigItem(
            "General", "RetryAttempts", 3, RangeValidator(0, 10))
        ...
        # Network settings
        self.use_proxy = ConfigItem(
            "Network", "UseProxy", False, BoolValidator())
        self.proxy_host = ConfigItem("Network", "ProxyHost", "")
        self.proxy_port = RangeConfigItem(
            "Network", "ProxyPort", 8080, RangeValidator(1, 65535))
        self.timeout = RangeConfigItem(
            "Network", "Timeout", 30, RangeValidator(10, 300))
        self.user_agent = OptionsConfigItem(
            "Network", "UserAgent", "default",
            OptionsValidator(
                ["default", "chrome", "firefox", "safari", "custom"])
        )
...
class SettingsManager:
    ...
    def load_from_settings(config: SettingsConfig, settings):
        ...
        try:
            # General settings
            config.output_directory.value = settings.get(
                "general", "output_directory", str(Path.home() / "Downloads"))
            config.auto_cleanup.value = settings.get(
                "general", "auto_cleanup", True)
            config.auto_start_downloads.value = settings.get(
                "general", "auto_start_downloads", True)
            config.check_updates.value = settings.get(
                "general", "check_updates", True)
            config.enable_notifications.value = settings.get(
                "general", "enable_notifications", True)
            config.max_recent_files.value = settings.get(
                "general", "max_recent_files", 10)
            config.max_concurrent_downloads.value = settings.get(
                "general", "max_concurrent_downloads", 3)
            config.retry_attempts.value = settings.get(
                "general", "retry_attempts", 3)

            # Sync UI versions
            config.max_concurrent_downloads_ui.value = str(
            ...
            # Network settings
            config.use_proxy.value = settings.get(
                "network", "use_proxy", False)
            config.proxy_host.value = settings.get("network", "proxy_host", "")
            config.proxy_port.value = settings.get(
                "network", "proxy_port", 8080)
            config.timeout.value = settings.get("network", "timeout", 30)
            config.user_agent.value = settings.get(
                "network", "user_agent", "default")
            config.custom_user_agent.value = settings.get(
                "network", "custom_user_agent", "")
    ...
    def save_to_settings(config: SettingsConfig, settings):
        ...
        try:
            ...
            settings.set("general", "auto_start_downloads",
                         config.auto_start_downloads.value)
            settings.set("general", "check_updates",
                         config.check_updates.value)
            settings.set("general", "enable_notifications",
                         config.enable_notifications.value)
            settings.set("general", "max_recent_files",
                         config.max_recent_files.value)
            settings.set("general", "max_concurrent_downloads",
                         config.max_concurrent_downloads.value)
            settings.set("general", "retry_attempts",
                         config.retry_attempts.value)

            # Sync from UI versions
            config.max_concurrent_downloads.value = int(
                config.max_concurrent_downloads_ui.value)
            ...
            settings.set("network", "custom_user_agent",
