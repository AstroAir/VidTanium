...
class BatchConversionDialog(QDialog):
    ...
    def __init__(self, settings, parent=None):
        super().__init__("Batch Process", "", parent)
    ...
    def _create_ui(self):
        ...
        settings_layout.addRow("视频编码:", self.video_codec_combo)

        # 视频质量
        self.video_quality_combo = ComboBox()
        self.video_quality_combo.addItems(["高", "中", "低", "自定义"])
        settings_layout.addRow("视频质量:", self.video_quality_combo)
        ...
        settings_layout.addRow("自定义码率:", self.bitrate_spin)

        # 音频设置
        self.audio_codec_combo = ComboBox()
        self.audio_codec_combo.addItems(["copy", "aac", "mp3", "opus"])
        settings_layout.addRow("音频编码:", self.audio_codec_combo)
        ...
        settings_layout.addRow("", self.keep_original_check)
    ...
    def _add_folder(self):
        ...
        if folder:
            ...
            InfoBar.success(
                title="文件已添加",
                content=f"从文件夹中添加了 {self.files_list.count()} 个媒体文件",
    ...
    def _start_conversion(self):
        ...
        # 更新状态
        self.status_label.setText(f"转换完成。成功: {successful}, 失败: {failed}")
