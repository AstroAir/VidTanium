...
class TaskTable(TableWidget):
    ...
    def update_task_row(self, task_id, task) -> None:
        ...
        for row in range(self.rowCount()):
            ...
            if item and item.data(Qt.ItemDataRole.UserRole) == task_id:
                ...
                last_run_text = self._format_datetime(
                    task.last_run) if task.last_run else "--"
                last_run_item = self.item(row, 4)
                if last_run_item:
                    last_run_item.setText(last_run_text)
    ...
    def filter_tasks(self, search_text="", filter_type="all") -> None:
        ...
        for row in range(self.rowCount()):
            ...
            # 应用类型过滤
            # Get text safely with null checks
            status_text = status_item.text() if status_item else ""
            type_text = type_item.text() if type_item else ""

            if filter_type == "enabled" and "已禁用" in status_text:
                show_row = False
            elif filter_type == "disabled" and "已启用" in status_text:
                show_row = False
            elif filter_type == "one_time" and "一次性" not in type_text:
                show_row = False
            elif filter_type == "recurring" and "一次性" in type_text:
                show_row = False
