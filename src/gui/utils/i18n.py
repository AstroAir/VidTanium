...
class I18nManager:
    ...
    def _validate_translations(self) -> None:
        ...
        if not self.translations:
            logger.error(
                "No translations loaded! Application may not function properly.")
            return

        # Check if default locale exists
        if self.default_locale not in self.translations:
            logger.warning(
                f"Default locale '{self.default_locale}' not found. Available: {list(self.translations.keys())}")
        ...
        logger.info(
            f"I18n initialized with {len(self.translations)} locales: {list(self.translations.keys())}")
        ...
        try:
            ...
            for locale_file in self.locales_dir.glob("*.json"):
                ...
                try:
                    ...
                    logger.debug(
                        f"Loaded translations for locale: {locale_code}")
                except Exception as e:
                    logger.error(
                        f"Failed to load translation file {locale_file}: {e}")
    ...
    def set_locale(self, locale: str) -> bool:
        ...
        if locale in self.translations:
            ...
            logger.warning(
                f"Locale '{locale}' not available. Available locales: {list(self.translations.keys())}")
            return False
    ...
    def _get_translation(self, key: str, locale: str, **kwargs) -> str:
        ...
        try:
            ...
            translation = self._get_nested_value(
                self.translations.get(locale, {}), key)
            ...
            if translation is None and locale != "en" and self.default_locale != "en":
                translation = self._get_nested_value(
                    self.translations.get("en", {}), key)

            # Fallback to key itself if still not found
            if translation is None:
                logger.warning(
                    f"Translation not found for key: {key} in any available locale")
            ...
            if kwargs:
                try:
                    ...
                    logger.warning(
                        f"Missing variable {e} for translation key: {key}")
                except Exception as e:
                    logger.warning(
                        f"Error formatting translation for key {key}: {e}")
...
def tr(key: str, **kwargs) -> str:
    ...
    try:
        ...
        if not manager.is_initialized():
            logger.warning(
                "I18n manager not properly initialized, returning fallback")
            return key.split('.')[-1].replace('_', ' ').title()
