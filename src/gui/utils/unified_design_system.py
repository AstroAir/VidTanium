...
class UnifiedDesignSystem:
    """Unified Design System with optimized compact spacing"""
    ...
    # Optimized compact spacing - reduced by ~25% for more efficient use of screen space
    SPACING = {"xs": 4, "sm": 6, "md": 12, "lg": 18, "xl": 24, "xxl": 36, "xxxl": 48}
    ...
    def get_padding(cls, variant: str = "normal") -> Dict[str, int]:
        """Get padding configuration - optimized for compact layouts"""
        presets = {
            "compact": {"top": cls.spacing("xs"), "right": cls.spacing("sm"), "bottom": cls.spacing("xs"), "left": cls.spacing("sm")},
            "normal": {"top": cls.spacing("sm"), "right": cls.spacing("md"), "bottom": cls.spacing("sm"), "left": cls.spacing("md")},
            "spacious": {"top": cls.spacing("md"), "right": cls.spacing("lg"), "bottom": cls.spacing("md"), "left": cls.spacing("lg")}
        }
        return presets.get(variant, presets["normal"])
    @classmethod
    def get_gap(cls, variant: str = "normal") -> int:
        """Get gap/spacing between elements - optimized for compact layouts"""
        gaps = {"compact": cls.spacing("xs"), "normal": cls.spacing("sm"), "spacious": cls.spacing("md")}
        return gaps.get(variant, cls.spacing("sm"))
    @classmethod
